...
modelo  <- lgb.train( data= dtrain,
                      param= list( objective=        "binary",
                                   ...
                                   lambda_l2= 0.0,      
                                   max_bin=             31,
                                   learning_rate=        0.0101110885300797,
                                   num_iterations=      456,
                                   num_leaves=          1022,
                                   feature_fraction=     0.485571363238613,
                                   min_data_in_leaf=  3984,
                                   seed=            999983,
                                   force_row_wise= TRUE)  )
...
entrega  <- as.data.table( list( "numero_de_cliente"= dapply[  , numero_de_cliente],
                                 "Predicted"= as.integer(prediccion > 0.0159126384027532 ) )  ) #genero la salida
